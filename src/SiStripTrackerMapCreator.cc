...
void SiStripTrackerMapCreator::setTkMapFromHistogram(DQMStore* dqm_store, std::string& htype) {
  ...
  std::string mechanicalview_dir = dqm_store->pwd();
  //std::cout << "+++++++++++mechanicalview_dir: " << mechanicalview_dir << std::endl;
  ...
  for (std::vector<std::string>::const_iterator it = subdet_folder.begin(); it != subdet_folder.end(); it++) {
    ...
    //std::cout << "+++++ dirname: " << dname << std::endl;
    if (!dqm_store->dirExists(dname)) continue;
    dqm_store->cd(dname);
    //if (topNmodules) std::vector<std::pair<float,uint32_t> >* topNmodVec = new std::vector<std::pair<float,uint32_t> >;
	...
	if (htype == "QTestAlarm" ){
	  ...
	  tkhmap_me = (*itkh);
	  //std::cout << "----- QTestAlarm " << "fval: " << (*itkh) << std::endl;
	  break;
	} else if (name.find(htype) != std::string::npos) {
	  tkhmap_me = (*itkh);
	  //std::cout << "-----------------" << "fval: " << (*itkh) << std::endl;
	  break; 
	} 
      }
      if (tkhmap_me != 0) {
...
void SiStripTrackerMapCreator::printTopModules(std::vector<std::pair<float,uint32_t> >* topNmodVec){
   ...
   for (std::vector<std::pair<float,uint32_t> >::const_iterator itNmod = topNmodVec->begin(); itNmod != topNmodVec->end(); itNmod++){
       ...
       edm::LogVerbatim("TopModules") << aPair.second << " " << aPair.first;
   }
   edm::LogVerbatim("TopModules") << "-----------------------------------------";
}

void SiStripTrackerMapCreator::paintTkMapFromHistogram(DQMStore* dqm_store, MonitorElement* me, std::string& htype,/*bool doTopNmod,*/ std::vector<std::pair<float,uint32_t> >* topNmodVec) {
  ...
  //if(topNmodules) 
  //std::vector<std::pair<float,uint32_t> > topNmodulesVec;
  //topNmodulesVec.reserve(10000);
...
//    std::cout << "-----------detId: " << det_id << "  " << "fval: " << fval << std::endl;
    if (htype == "QTestAlarm") {
      edm::LogError("ItShouldNotBeHere") << "QTestAlarm map: you should not be here!";
      /*
      int rval, gval, bval;
